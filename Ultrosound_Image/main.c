...
/* ########################################## */
/* UDP */
#define SERVER_LOCAL_IP "169.254.230.97"
...
int main(int argc, char **arg){
    ...
    float servo_end = SE;
    float servo_step = (servo_end-servo_start)/(servo_resolution-1);

    for (int ii = 0; ii < servo_resolution; ii++){
        servo_angles[ii] = servo_start+servo_step*ii;
        //printf("%f\n", servo_start+servo_step*ii);
    }
    ...
    FILE * fp;
    fp = fopen("./RedPitaya_WhiteFin_V3.7.1_V0.8_Original_Glycerin+30mmLens+Lid+Gel+Servo(105~185)100+LX(0~180)90+Video_1.dat", "w");
    // set trigger delay
